(function () {
	...
	window.addEventListener('load', function () {
		...
		if (localStorage.getItem('height') == null || /\bheight=\d+/.test(location.search)) {
			localStorage.setItem('height', js2me.config.height);
		}
		if (localStorage.getItem('width') == null || /\bwidth=\d+/.test(location.search)) {
		...
		if (js2me.config.src) {
			screen.innerHTML = '<div id="jar-list">JAR Loading...</div>';
		} else if (js2me.config.selector) {
		...
		if (js2me.config.src) {
			...
			if (js2me.config.selector) {
				document.getElementById('file-selector').addEventListener('click', function () {
					js2me.chooseFile(function (files) {
						if (files.length) { 
							js2me.launchJAR(files[0]);
						}
					});
				});
			} else {

...
public final class RenderPipe {
	...
	public final void clear() {
		roList.removeAllElements();
		parentNode = null;
	}

	public final RenderObject getRenderObj(int index) {
		return (RenderObject) roList.elementAt(index);
	}
		...
		if (isVisible(node)) {
			...
			if (node instanceof Sprite3D) {
				...
				if (spr.getAppearance() != null && spr.getCropWidth() != 0 && spr.getCropHeight() != 0) {
					roList.addElement(new RenderObject(node, trans, 0, this));
				}
			} else {
				if (node instanceof Mesh) {
					...
					for (int i = 0; i < submeshes; ++i) {
						if (((Mesh) node).getAppearance(i) != null) {
							roList.addElement(new RenderObject(node, trans, i, this));
						}
					}
	...
	public void sortNodes() {
		// Simple bubble sort for CLDC compatibility
		int n = roList.size();
		for (int i = 0; i < n - 1; i++) {
			for (int j = 0; j < n - i - 1; j++) {
				RenderObject ro1 = (RenderObject) roList.elementAt(j);
				RenderObject ro2 = (RenderObject) roList.elementAt(j + 1);
				if (ro1.sortKey > ro2.sortKey) {
					roList.setElementAt(ro2, j);
					roList.setElementAt(ro1, j + 1);
				}
			}
		}
	}

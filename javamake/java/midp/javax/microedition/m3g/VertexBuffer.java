...
public class VertexBuffer extends Object3D {
	...
	public void uploadToGL(IndexBuffer ib, boolean flatShade, boolean uploadNormals, MeshProgram mp, BufferHelper bufferHelper) {
		...
		if (invalidated || uploadedFlat != flatShade ||
			...
			GLES2.bindBuffer(GL_ARRAY_BUFFER, bufferHandle);
			...
			GLES2.bufferSubData(GL_ARRAY_BUFFER, 0, neededSize, bufferBuffer);
		...
		if (vaoHandle == null) {
			...
			GLES2.vertexAttribPointer(mp.aVColor, 4, GL_UNSIGNED_SHORT, true, stride, 4*2*4);

			GLES2.enableVertexAttribArray(mp.aVNormal);
			GLES2.vertexAttribPointer(mp.aVNormal, 3, GL_SHORT, true, stride, 4*2*1);

			GLES2.enableVertexAttribArray(mp.aVPos);
			GLES2.vertexAttribPointer(mp.aVPos, 3, GL_SHORT, false, stride, 0);

			GLES2.enableVertexAttribArray(mp.aVPosLight);
			GLES2.vertexAttribPointer(mp.aVPosLight, 3, GL_SHORT, false, stride, 4*2*5);

			GLES2.enableVertexAttribArray(mp.aVCoords[0]);
			GLES2.vertexAttribPointer(mp.aVCoords[0], 2, GL_SHORT, false, stride, 4*2*(2+0));

			GLES2.enableVertexAttribArray(mp.aVCoords[1]);
			GLES2.vertexAttribPointer(mp.aVCoords[1], 2, GL_SHORT, false, stride, 4*2*(2+1));

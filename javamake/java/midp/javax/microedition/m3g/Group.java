...
public class Group extends Node {
	...
	protected Object3D duplicateObject() {
		...
		for (int i = 0; i < children.size(); i++) {
			clone.children.addElement(children.elementAt(i));
		}
		...
		for (int i = clone.getChildCount() - 1; i >= 0; --i) {
			...
			clone.children.setElementAt(newChild, i);
		}
	...
	protected void alignment(Node reference) {
		...
		for (int i = 0; i < children.size(); ++i) {
			((Node) children.elementAt(i)).alignment(reference);
		}
	}
	...
	protected void updateAlignReferences() {
		...
		for (int var1 = 0; var1 < this.children.size(); ++var1) {
			((Node) this.children.elementAt(var1)).updateAlignReferences();
		}
	...
	protected void clearAlignReferences() {
		...
		for (int var1 = 0; var1 < this.children.size(); ++var1) {
			((Node) this.children.elementAt(var1)).clearAlignReferences();
		}

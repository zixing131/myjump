// from kemulator.. consider swapping with a more open source version

package kemulator.m3g.impl;

// import org.recompile.mobile.Mobile; // CLDC doesn't support this
...
// import java.util.zip.Inflater; // CLDC doesn't support this
...
public final class M3GLoader {
	...
	private Object3D[] method746(String var1) throws IOException {
		if (var1 == null) {
			...
			int var3 = method756(var2 = new PeekInputStream(this.getInputStream(var1), 12));
			var2.rewind();
		...
		if (this.anInt1084 > 1 && this.aBoolean1090 && !this.aBoolean1085) {
			...
			if ((var3 = method705(var2 = new AdlerInputStream(var1))) == -1) {
				...
				if (var3 == 0) {
					var9 = var2;
					if (var7 != var5 - 13L) {
						System.err.println("M3GLoader: Section length mismatch!!!");
						return false;
					}
				} else {
					if (var3 != 1) {
						System.err.println("M3GLoader: Unrecognized compression schemeh(" + var3 + ")!!!");
						return false;
					}
	...
	private static int method730(byte[] var0, int var1) {
		...
		if (var4 == 0) {
			...
			if (var4 == 0) {
				...
				System.err.println("M3GLoader:Invalid file type, use png instead");
				return 2;
			}
		}
	}
	...
	private InputStream getInputStream(String name) throws IOException {
		...
		if (name.charAt(0) == '/' || this.aString1089 == null)   // absolute file reference
		{
			// CLDC doesn't support Mobile.getPlatform().loader.getResourceAsStream()
			// Use Class.getResourceAsStream() instead
		...
		String uri = this.aString1089.substring(0, this.aString1089.lastIndexOf('/') + 1) + name;

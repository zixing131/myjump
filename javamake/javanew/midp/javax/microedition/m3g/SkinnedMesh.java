...
public class SkinnedMesh extends Mesh {
	...
	protected Object3D duplicateObject() {
		...
		for (int i = 0; i < boneTransList.size(); i++) {
			BoneTransform oldBoneTrans = (BoneTransform)boneTransList.elementAt(i);
			BoneTransform newBoneTrans = new BoneTransform((Node)oldToNewBone.get(oldBoneTrans.bone), oldBoneTrans.toBoneTrans);

			clone.boneTransList.addElement(newBoneTrans);
		}

		clone.vtxBones = (int[])vtxBones.clone();
		clone.vtxWeights = (int[])vtxWeights.clone();
	...
	public void addTransform(Node bone, int weight, int firstVertex, int numVertices) {
		...
		for (int i = 0; i < boneTransList.size(); i++) {
			BoneTransform tmpBoneTrans = (BoneTransform)boneTransList.elementAt(i);
			...
			if (boneTrans == null) {
				...
				boneTransList.addElement(boneTrans);
